...
app.post('/api/guilds/:guildId/settings',auth,(req,res)=>{const guildId=req.params.guildId;if(!allowed(req,guildId))return res.sendStatus(403);for(const [k,v] of Object.entries(req.body))if(['recruitment_channel','logs_channel'].includes(k))setSetting(guildId,k,v);res.json({ok:true});});
