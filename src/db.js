...
CREATE TABLE IF NOT EXISTS tickets (id INTEGER PRIMARY KEY AUTOINCREMENT,guild_id TEXT NOT NULL,type TEXT NOT NULL,user_id TEXT,username TEXT NOT NULL,subject TEXT NOT NULL,details_json TEXT NOT NULL,status TEXT NOT NULL DEFAULT 'open',discord_channel_id TEXT,created_at TEXT NOT NULL,updated_at TEXT NOT NULL,closed_at TEXT);
...
export function setApplicationTicket(id,guildId,channelId,messageId){db.prepare('UPDATE applications SET ticket_channel_id=?,ticket_message_id=?,updated_at=? WHERE id=? AND guild_id=?').run(channelId||null,messageId||null,new Date().toISOString(),id,guildId);return getApplication(id,guildId)}
...
export function createTicket(d){const now=new Date().toISOString();const r=db.prepare('INSERT INTO tickets(guild_id,type,user_id,username,subject,details_json,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)').run(d.guildId,d.type,d.userId||null,d.username||'Visiteur',d.subject,d.details?JSON.stringify(d.details):'{}','open',now,now);return db.prepare('SELECT * FROM tickets WHERE id=?').get(r.lastInsertRowid)}
